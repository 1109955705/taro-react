...
export default abstract class QNBleNative extends QNBleNativeCommonExtends {
   ...
   */
  abstract createBleConnection(
    bleDevice: QNBleDevice,
    connectFailOverTime?: number
  ): Promise<any>;
  /**
   * 关闭蓝牙连接
   * @param {QNBleDevice} bleDevice - 轻牛蓝牙对象
   ...
   * @returns {Promise<any>}
   */
  abstract findBleDeviceCharacteristics(
    deviceId: string,
    serviceId: string
  ): Promise<any>;
  /**
   ...
   * @returns {Promise<any>}
   */
  abstract readBleCharacteristicValue(
    deviceId: string,
    serviceId: string,
    characteristicId: string
  ): Promise<any>;
   ...
   * @param {number[]} data
   */
  abstract doAdvertising(
    server: QNBleTypings.QNBleServer,
    data: number[]
  ): Promise<any>;
  /**
   * 停止广播

...
export interface QNBleProtocolProfile {
     ...
     * 构建目标设备类型
     */
    buildTargetDevice: (nativeDevice: QNBleNativeDevice) => Partial<QNBleDevice> | null;
}
export default class QNBleProtocol<T extends Partial<QNBleTypings.QNDeviceEventListener>> extends QNBleCommonExtends {
    /**
     * 对该设备的定义
     */
     ...
     */
    onGetData(payload: QNBleTypings.TypedCharacteristicValueChangePayload): Promise<any>;
    /**
     * 解析命令
 ...
 * 处理细分业务流的协议类
 */
export declare abstract class QNBleActionFlow<T extends Partial<QNBleTypings.QNDeviceEventListener>> {
    /**
     * 可以处理的action
     */
    ...
    writeCmd(serviceUUID: string, writeUUID: string, header: number, byteList: Array<number>): Promise<any>;
    log(...objs: any): void;
    /**
     * 计算体重
     * @param {*} height 高八位
    ...
    abstract decode(payload: Partial<QNBleTypings.TypedCharacteristicValueChangePayload>): Promise<any>;
}
export declare class QNBleBaseScaleProtocol<T extends QNBleTypings.QNBaseScaleMeasureEventListener> extends QNBleProtocol<T> {

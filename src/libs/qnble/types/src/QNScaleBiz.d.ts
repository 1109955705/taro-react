...
export default class QNScaleBiz extends QNBleCommonExtends {
  ...
  constructor(
    hostProtocol: QNBleProtocol<QNBleTypings.QNBaseScaleMeasureEventListener>
  );
  /**
   ...
   */
  fetchScaleData(
    scaleData: QNBleTypings.TypedScaleData
  ): Promise<QNBleTypings.TypedMeasure>;
  private makeMeasures;
  prepareData(data: QNBleTypings.TypedMeasureData): QNBleTypings.TypedMeasure;
  prepareWspMeasureData(
    data: QNBleTypings.TypedWspMeasureData
  ): QNBleTypings.TypedMeasure;
  /**
   * 得到体重数据
   * @param {number} a1
   * @param {number} b1
